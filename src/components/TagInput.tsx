...
const TagInput: React.FC<{
  ...
  const handleInputChange: ChangeEventHandler<HTMLInputElement> = (e) => {
    ...
    const newText = text.replaceAll(
      /([^,]+),/g,
      (wholeMatch: string, tagText: string) => {
  ...
  return (
    ...
    <div
      className={`
        relative rounded overflow-hidden p-1.5 sm:p-2 
        border border-gray-500 outline outline-0 
        focus-within:border-blue-950 focus-within:outline-blue-500/30 focus-within:outline-2
    ...
    >
      <ul className="flex items-center flex-wrap gap-1 sm:gap-1.5">
        ...
        <li className="relative flex-1 first:ml-1">
          ...
          />
          <span className="opacity-0 whitespace-pre h-7 sm:h-8 flex items-center justify-center">
            {inputText}&nbsp;
          </span>
        </li>
      </ul>
...
const TagItem: React.FC<{
  ...
  const divClassName = isUpForDeletion
    ? "scale-100 bg-red-300 text-red-950 shadow-red-500"
    : "scale-[0.8] bg-blue-200 text-blue-950 shadow-blue-300";

  return (
    <li
      className={`
        bg-blue-100 text-blue-950 h-7 sm:h-8 px-2 rounded 
    ...
    >
      ...
      <button
        type="button"
        className="relative z-30 group w-6 h-6 flex items-center justify-center rounded-full outline-none"
        title="Delete"
      ...
      >
        ...
        <svg
          className={`
            relative z-10 w-4 h-4 rounded overflow-hidden
